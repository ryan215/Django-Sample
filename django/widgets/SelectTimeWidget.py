...
class SelectTimeWidget(Widget):
    ...
    def render(self, name, value, attrs=None):
        try:
            # when grabbing the object format is in datetime
            hour_val, minute_val, second_val = value.hour, value.minute, value.second
        except:
            # when saving the object, widget gives "00:00:00" format
                # in order to keep consistancy we add this catch
            value = value.split(':')
            hour_val = int(value[0])
            minute_val = int(value[1])
            second_val = int(value[2])
